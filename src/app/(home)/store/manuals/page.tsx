...
import fetchStoreManuals from "@/auth/get-store-manuals/route";
...
async function getManualStore(currentPage: number) {
  try {
    const response = await fetchStoreManuals(currentPage);
...
export default async function Page({ searchParams }: { searchParams: { page?: string } }) {
  ...
  const data = await getManualStore(currentPage)
      ...
      <div className="flex w-full flex-col md:flex-row justify-center gap-3 md:justify-center md:gap-6 mb-8">
        ...
        <div className="w-11/12 mx-auto md:mx-0 md:w-2/3 lg:w-1/2 flex flex-col gap-3">
          <div className=" grid md:grid-cols-2 grid-cols-1 md:gap-4 gap-9">
            {data?.manuals ? (data?.manuals.map((product, index) => (
              <StoreCard {...product} category={"manuals"} key={index} />
            )))
              :
              <div className="flex items-center justify-center">
                <h1 className="text-2xl text-red-500">Ainda não possui manuais.</h1>
              </div>
            }
          </div>
          {
