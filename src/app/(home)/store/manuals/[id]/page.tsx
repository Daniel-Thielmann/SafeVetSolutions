...
async function getManualsId({ id }: { id: number }) {
  try {
    const response = await fetchSingleManual(id);
...
export default async function SingleProductView({ params }: { params: { id: string } }) {
  ...
  const addToCart = async () => {
    ...
    if (!currentUser) {
      ...
      handleAddToCart(data?.manual.product)
    }
  ...
  const data = await getManualsId({ id })
      ...
      <div>
        ...
        <div className="flex w-full justify-center items-center flex-col gap-12">
          <div className="flex flex-col md:flex-row w-10/12 justify-center items-center gap-12">
            ...
            <div className=" w-full md:w-2/3 flex flex-col justify-center md:justify-start gap-5 lg:gap-12">
              <div className="flex flex-col gap-4">
                <h1 className="font-bunken font-bold text-center md:text-start text-[#192355] text-lg lg:text-xl">{data?.manual.product.title}</h1>

              </div>
              <span className="font-goldplay text-center md:text-start text-[#192355] text-2xl">R$ {data?.manual.product.price}</span>
          ...
          <div className="px-1 md:px-0 flex flex-col w-full md:w-10/12 gap-3 md:gap-5">
            ...
            {
              data?.manual &&
              <div className="text-[#192355] text-md font-goldplay mx-4 md:mx-0" dangerouslySetInnerHTML={{ __html: data?.manual.product.description }}></div>
            }
          </div>
        </div>
      </div>
