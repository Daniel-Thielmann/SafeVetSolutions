...
async function getConsultanciesId({ id }: { id: number }) {
  try {
...
export default async function SingleProductView({ params }: { params: { id: string } }) {
  ...
  const addToCart = async () => {
    ...
    if (!currentUser) {
      ...
      handleAddToCart(data?.consultancy.product)
    }
  ...
  const data = await getConsultanciesId({ id });

  return (
    <Suspense fallback={<div className="flex items-center justify-center text-3xl">Carregando...</div>}>
        ...
        <div>
          ...
          <div className="flex w-full justify-center items-center flex-col gap-12">
            <div className="flex flex-col md:flex-row w-10/12 justify-center items-center gap-12">
              <img
                src={data?.consultancy.product.image}
                width={500}
                height={500}
                alt="imagem"
              ...
              <div className=" w-full md:w-2/3 flex flex-col justify-center md:justify-start gap-5 lg:gap-12">
                <div className="flex flex-col gap-4">
                  <h1 className="font-bunken font-bold text-center md:text-start text-[#192355] text-lg lg:text-xl">{data?.consultancy.product.title}</h1>

                </div>
                <span className="font-goldplay text-center md:text-start text-[#192355] text-2xl">R$ {data?.consultancy.product.price}</span>
            ...
            <div className="px-1 md:px-0 flex flex-col w-full md:w-10/12 gap-3 md:gap-5 mb-4">
              ...
              {
                data?.consultancy &&
                <div className="text-[#192355] text-md font-goldplay mx-4 md:mx-0" dangerouslySetInnerHTML={{ __html: data?.consultancy.product.description }}></div>
              }
            </div>
          </div>
